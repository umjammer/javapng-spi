...
class RegisteredChunks {
    ...
    public static boolean read(int type, DataInput in, int length, PngImage png)
        ...
        case PngConstants.IHDR:
            read_IHDR(in, length, props);
            break;
        case PngConstants.IEND:
            checkLength(PngConstants.IEND, length, 0);
            break;
        case PngConstants.PLTE:
            read_PLTE(in, length, props, png);
            break;
        case PngConstants.bKGD:
            read_bKGD(in, length, props, png);
            break;
        case PngConstants.tRNS:
            read_tRNS(in, length, props, png);
            break;
        case PngConstants.sBIT:
            read_sBIT(in, length, props, png);
            break;
        case PngConstants.hIST:
            read_hIST(in, length, props, png);
            break;
        case PngConstants.sPLT:
            read_sPLT(in, length, props, png);
            break;
        case PngConstants.cHRM:
            read_cHRM(in, length, props);
            break;
        case PngConstants.gAMA:
            read_gAMA(in, length, props);
            break;
        case PngConstants.iCCP:
            read_iCCP(in, length, props);
            break;
        case PngConstants.pHYs:
            read_pHYs(in, length, props);
            break;
        case PngConstants.sRGB:
            read_sRGB(in, length, props);
            break;
        case PngConstants.tIME:
            read_tIME(in, length, props);
            break;
        case PngConstants.gIFg:
            read_gIFg(in, length, props);
            break;
        case PngConstants.oFFs:
            read_oFFs(in, length, props);
            break;
        case PngConstants.sCAL:
            read_sCAL(in, length, props);
            break;
        case PngConstants.sTER:
            read_sTER(in, length, props);
            break;
